  ...
  <div class="min-h-screen px-5 pb-12 bg-center bg-cover pt-28 lg:pt-12" style="background-image: url({{ $background }})">
      <div class="max-w-5xl p-5 mx-auto bg-white rounded-lg">
				...
				<div class="flex items-center justify-between pb-5 overflow-hidden shadow-sm sm:rounded-lg">
					<div>
						<h3 class="text-xl font-semibold">Kelola Bus <span class="pl-2 text-sm font-normal text-indigo-700">{{ $count }} Bus</span></h3>
					</div>
					<form class="basis-1/2 md:basis-1/3">   
						...
						<div class="relative">
								...
								<a href="{{ route('dashboard.bus.create') }}" class="text-white absolute flex right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 ">
									New
						...
						<tbody>
								...
								<tr class="flex flex-row flex-wrap mb-10 bg-white lg:hover:bg-gray-100 lg:table-row lg:flex-row lg:flex-no-wrap lg:mb-0">
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Kode Bus</span>
											{{ $b->kode_bus }}
									</td>
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Nama & Spesifikasi Bus</span>
											<p>{{ $b->nama_bus }} <p class="text-sm text-gray-400">{{ $b->kapasitas_solar }} L - {{ $b->jumlah_kursi }} Kursi - Rp {{ number_format($b->harga_sewa, 0, ',', '.')}}</p></p>
									</td>
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Status Bus</span>
									...
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Kategori Bus</span>
									...
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											...
											<a href="{{ route('dashboard.bus.destroy', $b->kode_bus) }}" class="inline-block hover:text-red-600">
												@svg('bi-trash', 'w-5 h-5')
											</a>
											<a href="{{ route('dashboard.bus.edit', $b->kode_bus) }}" class="inline-block pl-2 hover:text-blue-600">
