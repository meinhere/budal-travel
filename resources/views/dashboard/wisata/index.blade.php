  ...
  <div class="min-h-screen px-5 pb-12 bg-center bg-cover pt-28 lg:pt-12" style="background-image: url({{ $background }})">
      <div class="max-w-6xl p-5 mx-auto bg-white rounded-lg">
				...
				<div class="flex items-center justify-between pb-5 overflow-hidden shadow-sm sm:rounded-lg">
					<div>
						<h3 class="text-xl font-semibold">Kelola Wisata <span class="pl-2 text-sm font-normal text-indigo-700">{{ $count }} Wisata</span></h3>
					</div>
					<form class="basis-1/2 md:basis-1/3">   
						...
						<div class="relative">
								...
								<a href="{{ route('dashboard.wisata.create') }}" class="text-white absolute flex right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 ">
									New
						...
						<tbody>
								...
								<tr class="flex flex-row flex-wrap mb-10 bg-white lg:hover:bg-gray-100 lg:table-row lg:flex-row lg:flex-no-wrap lg:mb-0">
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Nama Wisata</span>
											<p class="font-semibold">{{ $w->nama_wisata }} <p class="text-gray-500">{{ $w->kota->provinsi->nama_provinsi }}</p></p>
									...
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Harga Tiket</span>
											<span class="px-3 py-1 text-sm font-semibold rounded-full ">
												Rp {{ number_format($w->tarif_masuk_wisata, 0, ',', '.')}}
											</span>
									</td>
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											<span class="absolute top-0 left-0 px-2 py-1 text-xs font-bold uppercase bg-blue-200 lg:hidden">Jam Buka/Jam Tutup</span>
											{{\Carbon\Carbon::createFromFormat('H:i:s',$w->jam_buka)->format('H:i')}} - {{\Carbon\Carbon::createFromFormat('H:i:s',$w->jam_tutup)->format('H:i')}} WIB
									</td>
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
									...
									<td class="relative block w-full p-3 text-center text-gray-800 border border-b lg:text-left lg:w-auto lg:table-cell lg:static">
											...
											<a href="{{ route('dashboard.wisata.destroy', $w->kode_wisata) }}" class="inline-block hover:text-red-600">
												@svg('bi-trash', 'w-5 h-5')
											</a>
											<a href="{{ route('dashboard.wisata.edit', $w->kode_wisata) }}" class="inline-block pl-2 hover:text-blue-600">
